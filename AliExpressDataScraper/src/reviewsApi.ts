...
// {@link ReviewsSummary} DTO — the API-based replacement for the DOM scrape in `reviews.ts`.
//
...
import { fetchProductReviews, fetchSellerReviews } from './sellerApi.js';
import type { RatingBreakdown, ReviewSample, ReviewsSummary, SellerReviewSample } from './types.js';
 ...
 * the shape isn't recognized (e.g. an error `ret`) so the caller can fall back to the DOM scrape.
 */
 ...
 * when no star call yielded a recognizable response (caller can fall back to the DOM scrape).
 */
...
/**
 * Map one seller-store `evaViewList` entry to a {@link SellerReviewSample} — the lean subset shown
 * on the store's "Customer reviews" panel. The seller endpoint returns the same `evaViewList` shape
 * as the product reviews API, but we keep only the display fields here.
 */
function toSellerSample(raw: unknown): SellerReviewSample {
    const r = asRecord(raw);
    // `buyerEval` is a 0–100 score in steps of 20 (100 → 5 stars), so divide to recover the star value.
    const eval100 = toNumber(r.buyerEval);
    return {
...
/**
 * Parse a raw `evaluation.productEvaluation` (seller) response into a list of lean
 * {@link SellerReviewSample}s, or `null` when the shape isn't recognized (e.g. an error `ret`).
 */
export function parseSellerReviews(res: unknown): SellerReviewSample[] | null {
    const data = asRecord(asRecord(res).data);
    if (Object.keys(data).length === 0) {
        return null;
    }
    return Array.isArray(data.evaViewList) ? data.evaViewList.map(toSellerSample) : [];
}

/**
 * Collect a seller's store reviews per star rating: one API call per star, keeping at most `perStar`
 * samples from each. Mirrors {@link collectProductReviews} — the `filter` param selects a single star
 * (5→1). The MTOP calls share one JSONP callback, so they MUST run sequentially. Returns `null` when
 * no star call yielded a recognizable response.
 */
export async function collectSellerReviews(
    page: Page,
    sellerAdminSeq: string | number,
    log: Log,
    opts: { perStar?: number; stars?: number[] } = {},
): Promise<SellerReviewSample[] | null> {
    const perStar = opts.perStar ?? 5;
    const stars = opts.stars ?? [5, 4, 3, 2, 1];
    const samples: SellerReviewSample[] = [];
    let any = false;

    // Highest stars first so the most positive reviews lead the sample list.
    for (const star of stars) {
        const res = await fetchSellerReviews(page, sellerAdminSeq, log, { filter: star, pageSize: perStar });
        const parsed = parseSellerReviews(res);
        if (!parsed) {
            continue;
        }
        any = true;
        samples.push(...parsed.slice(0, perStar));
    }

    if (!any) {
        return null;
    }
    log.info('seller reviews collected', { perStar, samples: samples.length });
    return samples;
}
