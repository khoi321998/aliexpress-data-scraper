...
- **Avoidance:** each request runs real Chrome with a consistent, generated
  fingerprint (user-agent, navigator props, viewport, headers, `navigator.webdriver`
  hidden) over a sticky US residential proxy, plus human-like scrolling and timing.
- **Rotation:** if a captcha / punish / blocked / empty page is detected anyway, the
  Actor retires that session (abandoning the burned IP + fingerprint) and Crawlee
